...
test("profiles table includes categories field", () => {
  assert.match(
    schemaSource,
    /profiles:\s*defineTable\(\{[\s\S]*?categories:\s*v\.optional\(v\.array\(v\.string\(\)\)\)/,
    "profiles should have categories: v.optional(v.array(v.string()))"
  );
});

test("categories field appears after imagePrompt field in profiles", () => {
