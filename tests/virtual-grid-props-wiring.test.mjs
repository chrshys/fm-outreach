...
test("MapContentProps includes onSelectVirtual", () => {
  assert.match(mapContentSource, /onSelectVirtual\?:\s*\(cell:\s*VirtualCell\s*\|\s*null\)\s*=>\s*void/)
})
...
test("MapContent passes onSelectVirtual to DiscoveryGrid with default", () => {
  assert.match(mapContentSource, /DiscoveryGrid[\s\S]*onSelectVirtual=\{onSelectVirtual\s*\?\?/)
})
...
test("Map page passes onSelectVirtual to MapContent", () => {
  assert.match(mapPageSource, /<MapContent[\s\S]*onSelectVirtual=/)
})
